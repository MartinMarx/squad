...
describe('getTaskEnvVars', () => {
  ...
  it('returns all six env vars', () => {
    ...
    expect(vars).toHaveProperty('SQUAD_TASK_ID');
    expect(vars).toHaveProperty('SQUAD_TASK_NAME');
    expect(vars).toHaveProperty('SQUAD_TASK_PATH');
    expect(vars).toHaveProperty('SQUAD_ROOT_PATH');
    expect(vars).toHaveProperty('SQUAD_DEFAULT_BRANCH');
    expect(vars).toHaveProperty('SQUAD_PORT');
  });

  it('passes through taskId, taskPath, projectPath directly', () => {
    const vars = getTaskEnvVars(baseCtx);
    expect(vars.SQUAD_TASK_ID).toBe('task-abc-123');
    expect(vars.SQUAD_TASK_PATH).toBe('/home/user/worktrees/fix-login-bug');
    expect(vars.SQUAD_ROOT_PATH).toBe('/home/user/projects/myapp');
  });

  it('slugifies the task name', () => {
    const vars = getTaskEnvVars(baseCtx);
    expect(vars.SQUAD_TASK_NAME).toBe('fix-login-bug');
  });

  it('strips leading/trailing hyphens from slug', () => {
    const vars = getTaskEnvVars({ ...baseCtx, taskName: '---Hello World---' });
    expect(vars.SQUAD_TASK_NAME).toBe('hello-world');
  });

  it('falls back to "task" when name slugifies to empty', () => {
    const vars = getTaskEnvVars({ ...baseCtx, taskName: '!!!' });
    expect(vars.SQUAD_TASK_NAME).toBe('task');
  });

  it('defaults defaultBranch to "main"', () => {
    const { defaultBranch: _, ...ctx } = baseCtx;
    const vars = getTaskEnvVars(ctx as TaskEnvContext);
    expect(vars.SQUAD_DEFAULT_BRANCH).toBe('main');
  });

  it('uses provided defaultBranch', () => {
    const vars = getTaskEnvVars({ ...baseCtx, defaultBranch: 'develop' });
    expect(vars.SQUAD_DEFAULT_BRANCH).toBe('develop');
  });

  it('generates a port in the 50000-59990 range', () => {
    const vars = getTaskEnvVars(baseCtx);
    const port = Number(vars.SQUAD_PORT);
  ...
  it('generates stable port for same input', () => {
    ...
    expect(a.SQUAD_PORT).toBe(b.SQUAD_PORT);
  });

  it('uses portSeed over taskPath when provided', () => {
    const withSeed = getTaskEnvVars({ ...baseCtx, portSeed: 'custom-seed' });
    expect(typeof withSeed.SQUAD_PORT).toBe('string');
    expect(Number(withSeed.SQUAD_PORT)).toBeGreaterThanOrEqual(50000);
  });
});
