...
describe('config migration', () => {
  ...
  it('writes Conductor settings into .squad.json', async () => {
    const fs = createFs({
      'conductor.json': JSON.stringify({
        scripts: {
          setup: 'pnpm install',
    ...
    expect(JSON.parse(fs.content('.squad.json') ?? '{}')).toEqual({
      scripts: {
        setup: 'pnpm install',
        run: 'pnpm dev',
  ...
  it('writes Superset settings into .squad.json', async () => {
    const fs = createFs({
      '.superset/config.json': JSON.stringify({
        setup: ['bun install', 'cp "$SUPERSET_ROOT_PATH/.env" .env'],
        run: ['./.superset/run.sh'],
  ...
  it('writes Paseo settings into .squad.json', async () => {
    const fs = createFs({
      'paseo.json': JSON.stringify({
        worktree: {
          setup: 'npm ci',
  ...
  it('writes Codex local environment settings into .squad.json', async () => {
    const fs = createFs({
      '.codex/environments/environment.toml': `
        [setup]
        script = "npm ci"
  ...
  it('does not import when .squad.json already exists', async () => {
    const fs = createFs({
      'conductor.json': JSON.stringify({
        scripts: {
          run: 'pnpm dev',
      ...
      '.squad.json': JSON.stringify({ scripts: { run: 'pnpm dev' } }),
    });
    ...
    expect(result).toEqual({
      ...
      error: {
        type: 'write-config-failed',
        message: '.squad.json already exists.',
      },
    });
    expect(fs.write).not.toHaveBeenCalled();
  });
