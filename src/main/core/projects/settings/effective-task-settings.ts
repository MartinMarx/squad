...
export async function getEffectiveTaskSettings(args: {
  ...
  const exists = await taskFs.exists('.squad.json');
  if (!exists) {
    return mergeShareableProjectSettings(defaults, localShareableSettings);
  }

  try {
    const { content } = await taskFs.read('.squad.json');
    ...
    log.warn('Failed to parse task .squad.json, falling back to project settings', err);
    return mergeShareableProjectSettings(defaults, localShareableSettings);
  }
}
