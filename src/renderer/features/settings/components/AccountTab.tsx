...
export function AccountTab() {
  ...
  const handleSignOut = () => {
    showConfirmSignOut({
      title: 'Sign out of Squad?',
      description: 'You will need to sign in again to reconnect your Squad account.',
        ...
        <div className="flex flex-col gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-xs">
              Sign in again to reconnect your Squad account.
            </p>
          </div>
  ...
  return (
    <div className="bg-muted/10 rounded-xl border border-border/60 p-4">
      <div className="flex flex-col gap-3">
        <div>
          <p className="text-sm font-medium text-foreground">Squad Account</p>
          <p className="text-muted-foreground text-xs">
            Create an Squad account to automatically connect GitHub using OAuth2.
          </p>
        </div>
        {error && <p className="text-destructive text-xs">{error}</p>}
        {serverAvailable === false ? (
