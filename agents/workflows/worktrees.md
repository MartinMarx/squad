...
- branch prefix defaults to `squad` and is configurable in app settings
...
`.squad.json` stores optional shareable project settings. Supported runtime keys:
...
Base project settings are DB-backed Project Settings, not runtime `.squad.json` keys:
...
- `preservePatterns` never copies tracked files or `.squad.json`
